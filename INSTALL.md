...
2. **双击 `安装.bat`** —— 唯一需要做的操作，全程自动：
   ...
   - 脚本会查找本机已有的 Python 3.10–3.12（**优先 3.10**）；
     找不到时会**自动下载并安装 Python 3.10**（先尝试 winget，失败则从
     python.org 静默安装）。
...
> 需要几分钟）。缺少 YOLO 依赖时，重新双击 `安装.bat` 即可补装。
...
- YOLO 检测器**自动匹配游戏窗口的真实客户区**（每几秒刷新一次，窗口移动/
  ...
  自动跟随，无需重新设置（会保存到 `yolo_detection_settings.json`）。
...
- **提示"地图名字不同 / map name does not match"** — 说明当前画面和已录制的地图
  不一致。默认配置**不绑定任何地图**（`recording-configuration.json` /
  `rope_calibration.json` 的地图名为空）：请先在当前地图上录制 最左/绳索/最右，
