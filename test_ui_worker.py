...
class UiLogHandlerTests(unittest.TestCase):
    ...
    def test_alerts_still_apply_when_shutdown_worker_is_none(self) -> None:
        ...
        # Scheduled shutdown is temporarily disabled, so the runtime passes
        # shutdown_worker=None.  The alarm/reminder wiring must NOT be
        # skipped by that: 掉线警报, 测谎警报, and 声音/闪烁/消息 toggles
        # must still reach their workers (they were silently dead because
        # _shutdown_apply_to_worker early-returned on worker None).
        class Var:
            def __init__(self, value):
                self.value = value
        ...
        worker = UiWorker.__new__(UiWorker)
        worker.shutdown_worker = None
        worker.character_worker = FakeCharacter()
        worker.lie_detector_worker = FakeLieDetector()
        worker.countdown_worker = FakeLieDetector()
        worker.screen_blinker = FakeBlinker()
        worker._shutdown_enabled_var = Var(False)
        worker._shutdown_hours_var = Var(3.0)
        worker._shutdown_hours_label = Label()
        worker._shutdown_slider = Slider()
        worker._shutdown_status = Label()
        worker._player_check_var = Var(False)
        worker._disconnect_alert_var = Var(True)
        worker._lie_alert_var = Var(True)
        worker._sound_alert_var = Var(False)
        worker._screen_blink_var = Var(True)
        worker._telegram_enabled_var = Var(False)
        worker._telegram_bot_token = ""
        worker._telegram_chat_id = ""
        worker._telegram_machine_var = Var("")

        def fake_path(self):
            return Path(tempfile.gettempdir()) / "test_none_worker_settings.json"
        ...
        self.assertEqual(worker.character_worker.sound_calls, [False])
        self.assertEqual(worker.lie_detector_worker.sound_calls, [False])
        self.assertEqual(worker.screen_blinker.calls, [True])
...
class WindowGeometryHelperTests(unittest.TestCase):
    ...
    def test_clamp_window_geometry_caps_initial_height(self) -> None:
        width, height, _, _ = _parse_window_geometry(
            _clamp_window_geometry(
                "1200x1800+40+40", 2560, 1440, max_height=900
            )
        )
