...
def main() -> int:
    ...
    with mss.MSS() as sct:
        while True:
            ...
            # Preview: character/environment/mob detections, each drawn with
            # its own color by _draw_detections (item/npc/ui excluded).  The
            # attack decision above still uses the mob-only tracked detections.
